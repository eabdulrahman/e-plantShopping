...
function ProductList({ onHomeClick }) {
    ...
    const addedToCartItems = useSelector(state => state.cart.addedToCart);
        ...
        <div>
            <div className="navbar" style={styleObj}>
                ...
                <div style={styleObjUl}>
                        ...
                        {/* Wrap the cart icon in a new div */}
                        <div className="cart-wrapper" style={{ position: 'relative', display: 'inline-block' }}>
                            <a href="#" onClick={(e) => handleCartClick(e)} style={styleA}>
                                <h1 className='cart'>
                                ...
                                <div className="product-list"> {/* Container for the list of plant cards */}
                                    {category.plants.map((plant, plantIndex) => (
                                        <div className='product-card' key={plantIndex}>
                                            <img className="product-image" src={plant.image} alt={plant.name} height={200}/>
                                            <p className="product-title">{plant.name}</p>
                                            <p className="product-description"><strong>Description: </strong>{plant.description}</p>
                                            <p className="product-cost"><strong>Unit price: </strong>{plant.cost}</p>
                                            <button 
                                            className={`product-button ${addedToCartItems[plant.name] ? "added-to-cart" : ""}`}
                                            onClick={() => {handleAddToCart(plant)}} disabled={addedToCartItems[plant.name]}>{addedToCartItems[plant.name] ?'Added to Cart' : 'Add to Cart'}</button>
                                        </div>
                                    ))}
                                </div>
                            </div>
